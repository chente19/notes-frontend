      ...
      <v-col cols="12">
        <v-card>
          <v-toolbar flat color="light-green lighten-4">
            <v-icon color="blue" left>mdi-account-plus</v-icon>
            <v-toolbar-title>ALTA DE USUARIO</v-toolbar-title>
            <v-spacer></v-spacer>
          </v-toolbar>
          <v-card-text>
            <v-form
                      ...
                      <template>
                        <v-container>
                          <v-alert
                            ...
                            >Usuario {{ userName }} guardado, No. de empleado
                            ...</v-alert
                          >
                          <br />
                          <v-alert
...
export default {
  ...
  methods: {
    ...
    async createUserNoEmail() {
      try {
        let resultGetter = this.$store.getters.currentUser;
        this.currentUser = resultGetter.user_number;
        this.userInterfaceNoEmail.user_name = this.userName;
        this.userInterfaceNoEmail.user_first_lastname = this.lastName1;
        this.userInterfaceNoEmail.user_second_lastname = this.lastName2;
        this.userInterfaceNoEmail.user_password = this.password;
        this.userInterfaceNoEmail.user_role = this.role;
        this.userInterfaceNoEmail.user_created_by = this.currentUser;
        // Aqui la peticion axios
        const backResponse = await axios.post(
          "user/add",
          this.userInterfaceNoEmail
        );
        const theData = await backResponse.data;
        const numberValueString = await theData.new_user_number;
        this.theNewNumberUser = numberValueString;
    ...
    async createUser() {
      try {
        ...
        this.userInterface.user_created_by = this.currentUser;
        // Aqui la peticion axios
        const backResponse = await axios.post("user/add", this.userInterface);
        const theData = await backResponse.data;
        const numberValueString = await theData.new_user_number;
        this.theNewNumberUser = numberValueString;
        this.showSuccessRequest = true;
    ...
    kindOfRequest() {
      // este metodo es para verificar si enviamos sin email o con email
      this.showCancelButton = false;
      if (this.needEmail) {
        this.createUser();
      } else {
        this.createUserNoEmail();
      }
    },
  },
};
